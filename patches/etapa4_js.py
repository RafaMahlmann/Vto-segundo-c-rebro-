...
# Localiza e substitui a funcao calcularDilacaoCasoEspecial + copiarRespostaCasoEspecial
import re

pattern = r"(function calcularDilacaoCasoEspecial\(\) \{)[\s\S]*?(function copiarRespostaCasoEspecial\(\) \{[\s\S]*?\}\n)"
...
new_js = r'''\1
    ...
    document.getElementById('respostaModelo1').value = modelo1Texto;
    document.getElementById('respostaModelo2').value = modelo2Texto;

    // Ativa a aba correta automaticamente
    if (dataPedido && !isNaN(diasTotais) && diasTotais >= 1 && !isNaN(diasConceder) && diasConceder >= 1) {
        mostrarModeloResposta(diasConceder >= diasTotais ? 2 : 1);
    }
}

function mostrarModeloResposta(num) {
    document.querySelectorAll('.modelo-resposta').forEach(el => el.classList.remove('ativa'));
    document.querySelectorAll('.mini-tab').forEach(el => el.classList.remove('ativa'));
    document.getElementById('modelo' + num).classList.add('ativa');
    document.getElementById('tabM' + num).classList.add('ativa');
}
...
result = re.sub(pattern, new_js, s)

if result != s:
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(result)
    print('Etapa 4 (JS controle de abas e dois modelos) concluida.')
else:
